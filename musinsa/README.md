...
무신사 MD와 브랜드 스카우트가 매주 뉴스룸, 기사, 플랫폼 콘텐츠, 브랜드 공식 사이트, 공개 커뮤니티에 흩어진 브랜드/협업/입점 후보 신호를 수동으로 훑어야 하는 일을 Codex가 자동 스크리닝해 `신규 검토`, `협업/콘텐츠 확장`, `모니터링`, `제외` 4분류 triage와 근거로 정리한다.
...
이번 예시 산출물 기준으로, 사람이 수동으로 하면 최소 아래 확인을 반복해야 한다. 숫자는 예시 파일에 기록된 후보 8개와 신호 24개 기준이다.

| 수동으로 하는 일 | 이번 예시에서 반복되는 확인 | 플러그인이 대신 산출하는 것 |
| --- | --- | --- |
| 후보별 공개 근거 모으기 | 8개 브랜드의 공식/플랫폼/기사 URL 확인 | `signals-log.csv` 24개 신호와 출처 강도 |
| 이미 알려진 브랜드와 신규 검토 후보 구분 | 무신사/29CM 노출·협업 이력 확인 | 4분류 triage 상태와 반대 근거 |
| 브랜드별 고객 반응 확인 | 브랜드마다 고객 반응, 플랫폼 큐레이션, 공개 성과 여부 확인 | `scorecards/*.md`의 고객 반응 신호와 `확인 필요` 표시 |
...
생성된 `dist/musinsa/submission.zip` 안의 `src/`가 플러그인 루트다. Codex에서 플러그인을 로드한 뒤 입력 예시처럼 호출한다.
