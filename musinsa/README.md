...
이 MVP는 API 키, 크롤링 권한, 무신사 내부 매출·검색·CRM·파트너센터 데이터 없이 공개 URL과 로컬 자막·예시 파일만으로 재현 가능하게 실증했다. 1인이 2일 안에 `src/` 플러그인, 근거 맵, 예시 산출물, validator 통과까지 만들 수 있는 범위이며, 사람의 최종 판단을 전제로 한다. 이 리소스 조건에서는 내부 예측 모델보다 MD 회의 전 공개 신호 감사 자동화가 최선이다. 제출 zip 안의 URL, 확인일, 반대 근거, 스코어카드로 자동화 범위를 직접 검증할 수 있고, 매출 예측·개인화 추천·파트너 운영 자동화는 내부 데이터 없이는 보조 도구 수준을 넘기 어렵기 때문이다.
...
실제 예시는 `examples/여성-컨템포러리-2026-07-08/`에 있다.
...
이번 예시 실행 1회는 `signals-log.csv`와 `scorecards/*.md` 기준으로 공개 URL 23곳을 탐색하고, 브랜드 8개와 신호 24건을 교차 확인하며, 각 판단에 출처 URL·확인일·출처 강도·반대 근거를 남겼다. 4분류 판정 초안은 `scout-report.md`, `signals-log.csv`, `watchlist.md`, `scorecards/*.md`로 제출 zip 안에서 검증할 수 있다. MD는 산출물 검토와 최종 판정만 하면 된다.
...
제출 zip 안의 `src/`가 플러그인 루트다. 공개 검증에 필요한 예시 산출물과 핵심 문서는 `src/` 아래에도 사본으로 포함한다.
...
3. 정상 입력 예시가 `examples/여성-컨템포러리-2026-07-08/`와 `src/examples/여성-컨템포러리-2026-07-08/`에 생성되어 있다.
